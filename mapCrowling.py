...
# 식당 리스트
parking_list = driver.find_elements(By.CSS_SELECTOR, 'div.text_area')
# 페이지 리스트
...
for btn in range(len(next_btn))[1:]:  # next_btn[0] = 이전 페이지 버튼 무시 -> [1]부터 시작
    ...
    names = driver.find_elements(By.CSS_SELECTOR, 'span.place_bluelink TYaxT')  # (3) 장소명

    for data in range(len(restaurant_list)):  # 주차장 리스트 만큼
        print(data)
        try:
            # 지번, 도로명 초기화
            jibun_address = ''
